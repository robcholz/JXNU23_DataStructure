...
int main() {
	std::cout << "1.DEC转换HEX：" << std::endl;
	Dto16(255);
	Dto16(510);
	std::cout << "3.算数表达式求值：{'3','+','4','*','5'}" << std::endl;
	auto eva_str_wrapper = std::make_unique<linkedlist<char>>();
	auto eva_str = eva_str_wrapper.get();
	create(eva_str, {'3', '+', '4', '*', '5'});
	evaluate(*eva_str);
	std::cout << "4.子字符串：test_string, i=5,len=6" << std::endl;
	auto test_str = create_linkstr("test_string");
	auto substr = substring(test_str, 5, 6);
	print(substr);
	std::cout << std::endl << "5.删除子字符串：test_string, i=5,len=6" << std::endl;
	delstring(test_str, 5, 6);
	print(test_str);
	std::cout << std::endl << "6. 查找第一次出现的位置：test_, t=es" << std::endl;
	std::cout << index(test_str, create_linkstr("es"));
	std::cout << std::endl << "7. 朴素匹配" << std::endl;
	std::cout<<"第一次的位置：testes_es_es_, t=es";
	...
	std::cout << std::endl << "8. KMP" << std::endl;
	int next[20];
