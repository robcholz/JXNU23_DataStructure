...
int main() {
	...
	std::cout << "1. 求a[left..right]最大数: ";
	std::cout << max(test, 13) << std::endl;

	std::cout << "2. 调整[left..right]中的奇数到左边 偶数到右边: ";
	std::cout << "Before: ";
	print(test, 13);
	relocate(test, 13);
	std::cout << "After: ";
	...
	std::cout << "3.1. 冒泡升序排序[left..right]: ";
	bubbleSort(test, 13);
	print(test, 13);
	std::cout << std::endl;
	std::cout << "3.2. 二分查找[left..right]中的key[8]: ";
	...
	print(list.get());
	std::cout << std::endl;
	std::cout << "4.1. 查找链表最大值: ";
	std::cout << max(list.get())->data << std::endl;
	std::cout << "4.2. 删除链表所有x: ";
	delallx(list.get(), 7);
	print(list.get());
	std::cout << std::endl;
	std::cout << "4.2. 删除链表直到只剩一个x: ";
